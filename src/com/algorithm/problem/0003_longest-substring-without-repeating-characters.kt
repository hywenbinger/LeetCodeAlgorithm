...
fun lengthOfLongestSubstring(string: String): Int {
    ...
    for ((index, value) in string.withIndex()) {
         ...
         *      随后再继续遍历到 a，发现 a 包含在 map 中，且 map[a]=0，如果像 (1) 一样处理，就会发现 left=map[a]+1=1，实际上 left 此时应该不变，left 始终为2，子段变成 ba 才对；
         * 为了处理以上两种情况，每次都应该更新 left=max(left , map[value]+1)
         * 另外，更新left后，不管原来的 value 是否在最长子段中，我们都要将 value 的位置更新为当前的 index，因此此时新的 value 已经进入到当前最长的子段中了。
         */
        ...
        println("$index-$value, left=$left, maxLen=$maxLen, map=$map")
    }
    return maxLen
}